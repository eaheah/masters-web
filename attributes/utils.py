    ...
    for d in related:
        ...
        if any([intermediary[reverse_label_dict[name]] == 1 for name in d]):
            for name in d:
                if name != 1:
                    print("get here?")
                    intermediary[reverse_label_dict[name]] == 0

    # if no related, pick strongest
    for d in related:
        l = [intermediary[reverse_label_dict[name]] for name in d]
        i = np.argmax(np.array(l))
        intermediary[reverse_label_dict[d[i]]] = 1
        print("STRONGEST")
        print(d[i])
